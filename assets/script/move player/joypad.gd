...
func process_input_event(_node: Node = null, event: InputEvent = null, button = false):
	...
	if event.axis in player_joypad:
		# filter the axis value so small unwanted move of the joypad don't trigger a move or a dirrection change in the animation
		...
		# Set the move variable of player to null if no move so the player can decelerate
		if move_scalar == Vector2.ZERO:
			_node.player.move = null
		else:
			# Set the player move to this so the player movement can be update itch frame
